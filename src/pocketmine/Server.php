...
class Server{
	 ...
	 * @return string
	 */
	public function getNightMoonApiVersion(){
	...
	public function getMotd(){
		return $this->getConfigString("motd", "NightMoon");
	}
 ...
 |_|  \__|_|\__  |_| |_|\__|_|   |_|\___/ \___/|_| |_|
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			if(file_exists($this->filePath . "src/pocketmine/resources/nightmoon_$lang.yml")){
				$content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/nightmoon_$lang.yml");
			}else{
				$content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/nightmoon_eng.yml");
			}

			if(!file_exists($this->dataPath . "nightmoon.yml")){
				@file_put_contents($this->dataPath . "nightmoon.yml", $content);
			}
			$internelConfig = new Config($file, Config::YAML, []);
			$this->advancedConfig = new Config($this->dataPath . "nightmoon.yml", Config::YAML, []);
			...
			if($cfgVer > $advVer){
				$this->logger->notice("Your nightmoon.yml needs update");
	...
	public function start(){
		...
		if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "NightMoon"))
			@mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "NightMoon");
